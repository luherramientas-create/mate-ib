...
const INSTITUTION_PATH = [
  'instituciones', 'liceoCariari', 'cursosLectivos', '2026', 'modalidades',
  'bachilleratoInternacional', 'niveles', '11', 'secciones'
];
...
export async function ensureAnonymousAuth() {
  if (authReady && auth.currentUser?.isAnonymous) return auth.currentUser;
      ...
      for (let index = 0; index < history.length; index += 1) {
        ...
        try {
          await saveAttemptToFirestore({
            ...
            highestHintLevel: 0,
            hintTypes: [],
            score: entry.score ?? calculateHistoricalScore(attemptNumber, Boolean(entry.correct)),
            timeSpent: part.timeSpent ?? null
          });
        } catch (error) {
...
export async function saveAttemptToFirestore({
  student, section, questionId, subquestionId, answer, correct,
  attemptNumber, hintsUsed, highestHintLevel, hintTypes, score, timeSpent
}) {
  const authUid = await getAnonymousUid();
  const syncId = `attempt-${authUid}-${questionId}-${subquestionId}-${attemptNumber}`;
  const attemptRef = doc(db, ...ASSESSMENT_PATH, student.id, 'preguntas', questionId,
    'subpreguntas', subquestionId, 'intentos', syncId);
...
export async function saveProgressToFirestore({ student, section, questionId, progress }) {
  const authUid = await getAnonymousUid();
  const progressRef = doc(db, ...ASSESSMENT_PATH, student.id, 'preguntas', questionId);
